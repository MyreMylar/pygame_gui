...
from pygame_gui._constants import UI_CHECKBOX_ON_HOVERED, UI_CHECKBOX_ON_UNHOVERED, OldType
from pygame_gui._constants import UI_CHECKBOX_TOGGLED, UI_CHECKBOX_START_CHECK
...
class UICheckbox(UIElement):
    """
    A checkbox, a lot of the appearance of the checkbox, including images to be displayed, is
    setup via the theme file. This checkbox is designed to be toggled or checked.

    The checkbox element is reused throughout the UI as part of other elements as it happens to be a
    ...
    :param relative_rect: Normally a rectangle describing the position (relative to its container) and
                          ...
                          will be dynamic depending on the checkbox's contents. Dynamic dimensions can
    ...
    :param allow_double_clicks: Enables double clicking on checkboxs which will generate a
                                unique event.
    ...
    def __init__(self, relative_rect: Union[pygame.Rect, Tuple[float, float], pygame.Vector2],
                ...
                previous_checked = False,  # Keep track of the previous state
                *,
        ...
        # support for an optional 'tool tip' element attached to this checkbox
        ...
        # A state that the checkbox can be in
        self.checked = False
        # Used to check checkbox toggle without going through pygame.Event system
        self.toggled_event = False
        self.held = False
        ...
        # default range at which we 'let go' of a checkbox
        self.hold_range = (0, 0)
    ...
    def hover_point(self, hover_x: int, hover_y: int) -> bool:
        """
        Tests if a position should be considered 'hovering' the checkbox. Normally this just means
        our mouse pointer is inside the checkboxs rectangle, however if we are holding onto the
        checkbox for a purpose(e.g. dragging a window around by it's menu bar) the hover radius can
    ...
    def can_hover(self) -> bool:
        """
        Tests whether we can trigger the hover state for this checkbox, other states take
        priority over it.
    ...
    def on_hovered(self):
        """
        Called when we enter the hover state, it sets the colours and image of the checkbox
        ...
        event_data = {'user_type': (UI_CHECKBOX_ON_HOVERED),
                      'ui_element': self,
        ...
        event_data = {'ui_element': self,
                      'ui_object_id': self.most_specific_combined_id}
        pygame.event.post(pygame.event.Event(UI_CHECKBOX_ON_HOVERED, event_data))

    def on_unhovered(self):
        """
        Called when we leave the hover state. Resets the colours and images to normal and kills any
        tooltip that was created while we were hovering the checkbox.
        """
        super().on_unhovered()
        ...
        # old event to remove in 0.8.0
        event_data = {'user_type': (UI_CHECKBOX_ON_UNHOVERED),
                      'ui_element': self,
                      'ui_object_id': self.most_specific_combined_id}
        pygame.event.post(pygame.event.Event(pygame.USEREVENT, event_data))
        ...
        event_data = {'ui_element': self,
                      'ui_object_id': self.most_specific_combined_id}
        pygame.event.post(pygame.event.Event(UI_CHECKBOX_ON_UNHOVERED, event_data))
    ...
    def update(self, time_delta: float):
        """
        Sets the checked state for an update cycle if we've toggled this checkbox recently.
        ...
        if self.alive():
            # clear toggle state, we only want it to last one update cycle
            self.checked = False

            if self.toggled_event:
                # if a toggle event has occurred set the checkbox to the checked state for one cycle.
                self.toggled_event = False
                self.checked = True

    def process_event(self, event: pygame.event.Event) -> bool:
        """
        Handles various interactions with the checkbox.
        :param event: The event to process.
        :return: Return True if we want to consume this event so it is not passed on to the rest of the UI.
        """
    ...
    def disable(self):
        """
        Disables the checkbox so that it is no longer interactive.
        """
        if self.is_enabled:
            ...
            # clear other checkbox state
            self.held = False
            self.checked = False
            self.is_selected = False
            self.toggle_event = False

    def enable(self):
        """
        Re-enables the checkbox so we can once again interact with it.
        """
    ...
    def _set_active(self):
        """
        Called when we are actively clicking on the checkbox. Changes the colours to the appropriate
        ones for the new state then redraws the checkbox.
        """
        self.drawable_shape.set_active_state('active')

    def _set_inactive(self):
        """
        Called when we stop actively clicking on the checkbox. Restores the colours to the default
        state then redraws the checkbox.
        """
        self.drawable_shape.set_active_state('normal')
    ...
    def unselect(self):
        ...
        to the default state then redraws the checkbox.
        """
        self.is_selected = False
        self.drawable_shape.set_active_state('normal')
    ...
    def set_text(self, text: str, *, text_kwargs: Optional[Dict[str, str]] = None):
        """
        Sets the text on the checkbox. The checkbox will rebuild.
    ...
    def set_hold_range(self, xy_range: Tuple[int, int]):
        """
        Set x and y values, in pixels, around our checkbox to use as the hold range for time when we
        want to drag a checkbox about but don't want it to slip out of our grasp too easily.

        Imagine it as a large rectangle around our checkbox, larger in all directions by whatever
    ...
    def in_hold_range(self, position: Union[pygame.math.Vector2,
        ...
        """
        Imagines a potentially larger rectangle around our checkbox in which range we still grip
        hold of our checkbox when moving the mouse. Makes it easier to use scrollbars.
        ...
        if self.drawable_shape.collide_point(position):
            ...
            hold_rect = pygame.Rect((self.rect.x - self.hold_range[0], self.rect.y - self.hold_range[1]),
                                    (self.rect.width + (2 * self.hold_range[0]), self.rect.height + (2 * self.hold_range[1])))
    ...
    def _calc_dynamic_size(self):
        if self.dynamic_width or self.dynamic_height:
            ...
            # if we have anchored the left side of our checkbox to the right of its container then
            ...
            # if we have anchored the top side of our checkbox to the bottom of it's container then
