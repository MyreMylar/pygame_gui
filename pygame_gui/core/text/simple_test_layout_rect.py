...
class SimpleTestLayoutRect(TextLayoutRect):
    ...
    def __init__(
        ...
        *,
        ...
        if create_split_points:
            self.split_points = [int(dimensions[0] / 3), int(dimensions[0] / 3) * 2]
        else:
            self.split_points = []
    ...
    def gen_random_colour():
        ...
        colour = Color("#000000")
        colour.hsla = (
            360 * ((random.uniform(1.0, 500.0) * golden_ratio) % 1),
            50,
            70,
            100,
        )
    ...
    def split(
        ...
        if current_split_point != 0:
            new_rect = SimpleTestLayoutRect(
                dimensions=(
                    self.width - current_split_point,  # noqa pylint: disable=no-member
                    self.height,
                ),
                create_split_points=True,
            )
