...
Use Notes: Mostly all that one will ever need is these 5 functions - **parse_colour_or_gradient_string**: Attempt to
parse a string that may be a colour or gradient into its respective value - **is_valid_colour_string**: Check if a
...
Developer Notes: How it Works: This module works through pairs of validating and parsing functions at the value,
colour, and gradient levels Hopefully everything should be generic enough that any new colour models or
representations can be easily added without much bloat or *magic*

        Parsing A Colour: Simply, The parsing of a colour is done by simply checking if there is any valid function
        cashed in the _colourParsers dictionary, then calling the paired parsing function to get its value Therefore,
        this system is not concrete at all, and should be extremely extensible to add **any 2 functions that can
        validate and parse a developer-determined schema**

        Parsing A Gradient: As of now, the gradient parser is implemented in such a way where it assumes that all
        commas outside an enclosing glyph ( Any comma not inside a (), [], or {} ) is a separator in a gradient list
        Generally, if creating new colour string schemas, this will break if there is a new colour which uses commas
        not enclosed in a glyph. This shouldn't be a problem right now, but it is worth noting as a warning in case
...
def validate_colour_model(strdata: str, name: str, types: List[NumParserType]) -> bool:
    """Use a colour model's name and types (generally denoted by its name in the _colourModelSchemas dictionary) to
    validate its use Developer Notes: - This function depends on the validator of each value type being present in
    the _valueParsers dictionary - This function assumes that the colour model also takes up the form "name(value,
    value...)"
...
def parse_colour_model(strdata: str, name: str, types: List[NumParserType]) -> List[T]:
    """Use a colour model's name and types (generally denoted by its name in the _colourModelSchemas dictionary) to
    return a tuple of its containing values Developer Notes: - This function depends on the parser of each value type
    being present in the _valueParsers dictionary - This function assumes that the strdata parameter has **already
    been validated** against the validate_colour_model function - This function assumes that the colour model also
    takes up the form "name(value, value...)"

    :param strdata: the colour model string to parse :type strdata: str
    :param name: the name of the colour model :type name: str
    :param types: the types of the values in the colour model in order, used to parse each value individually
                  :type types: List[NumParserType]
    :return: A tuple containing all the parsed values in the colour model :rtype: tuple :raises ValueError: if the
             colour model does not meet the standard <name(value, ...)> schema
    :raises KeyError: if the colour model cannot find the parser for a given type or the given type is not defined to
                      have a parser
    """
...
def is_valid_cmy_string(strdata: str) -> bool:
    """Validate CMY Color string in the format "cmy(percentage, percentage, percentage)" Value Parameter
    Descriptions: - **percentage**: either an integer value from 0 to 100 with "%" appended at the end or a float
    value ranging from 0 to 1 Examples: - "cmy(.4, .7, 80%)"
...
def parse_cmy_string(strdata: str) -> pygame.Color:
    """Parse CMY Color string in the format "cmy(percentage, percentage, percentage)" Value Parameter Descriptions: -
    **percentage**: either an integer value from 0 to 100 with "%" appended at the end or a float value ranging from
    0 to 1 Examples: - "cmy(.4, .7, 80%)"
...
def is_valid_hsl_string(strdata: str) -> bool:
    """Validate HSL Color string in the format "hsl(degree, percentage, percentage)" Value Parameter Descriptions: -
    **percentage**: either an integer value from 0 to 100 with "%" appended at the end or a float value ranging from
    0 to 1 - **degree**: a value between 0 and 360 with the "deg" unit optionally appended to the end Examples: -
    "hsl(30, 0.6, 0.7)" - "hsl(30deg, 40%, .5)"
...
def parse_hsl_string(strdata: str) -> pygame.Color:
    """Parse HSL Color string in the format "hsl(degree, percentage, percentage)" Value Parameter Descriptions: -
    **percentage**: either an integer value from 0 to 100 with "%" appended at the end or a float value ranging from
    0 to 1 - **degree**: a value between 0 and 360 with the "deg" unit optionally appended to the end Examples: -
    "hsl(30, 0.6, 0.7)" - "hsl(30deg, 40%, .5)"
...
def is_valid_hsla_string(strdata: str) -> bool:
    """Validate HSLA Color string in the format "hsla(degree, percentage, percentage, percentage)" Value Parameter
    Descriptions: - **percentage**: either an integer value from 0 to 100 with "%" appended at the end or a float
    value ranging from 0 to 1 - **degree**: a value between 0 and 360 with the "deg" unit optionally appended to the
    end Examples: - "hsla(30, 0.6, 0.7, 80%)" - "hsla(30deg, 40%, .5, 40%)"
...
def parse_hsla_string(strdata: str) -> pygame.Color:
    """Parse HSLA Color string in the format "hsla(degree, percentage, percentage, percentage)" Value Parameter
    Descriptions: - **percentage**: either an integer value from 0 to 100 with "%" appended at the end or a float
    value ranging from 0 to 1 - **degree**: a value between 0 and 360 with the "deg" unit optionally appended to the
    end Examples: - "hsla(30, 0.6, 0.7, 80%)" - "hsla(30deg, 40%, .5, 40%)"
...
def parse_hsv_string(strdata: str) -> pygame.Color:
    """Parse HSV Color string in the format "hsv(degree, percentage, percentage)" Value Parameter Descriptions: -
    **percentage**: either an integer value from 0 to 100 with "%" appended at the end or a float value ranging from
    0 to 1 - **degree**: a value between 0 and 360 with the "deg" unit optionally appended to the end Examples: -
    "hsv(50, 30%, .4)" - "hsv(60deg, 40%, 12%)"
...
def parse_hsva_string(strdata: str) -> pygame.Color:
    """Parse HSVA Color string in the format "hsva(degree, percentage, percentage, percentage)" Value Parameter
    Descriptions: - **percentage**: either an integer value from 0 to 100 with "%" appended at the end or a float
    value ranging from 0 to 1 - **degree**: a value between 0 and 360 with the "deg" unit optionally appended to the
    end Examples: - "hsva(50, .3, 40%, .75)" - "hsva(60deg, 40%, 12%, .94)"
...
def is_valid_hsva_string(strdata: str) -> bool:
    """Validate HSVA Color string in the format "*hsva(degree, percentage, percentage, percentage)*" Value Parameter
    Descriptions: - **percentage**: either an integer value from 0 to 100 with "%" appended at the end or a float
    value ranging from 0 to 1 - **degree**: a value between 0 and 360 with the "deg" unit optionally appended to the
    end Examples: - "hsva(50, .3, 40%, .75)" - "hsva(60deg, 40%, 12%, .94)"
...
def is_valid_colour_name(strdata: str):
    """| Validate Colour name string to be recognizable by pygame_gui as a valid colour_name | As of the writing of
    these documentations, all colour names defined are from the CSS colours given by all major browsers, which can be
    found here: https://w3schools.sinsixx.com/css/css_colornames.asp.htm | All colour names are not case-sensitive,
    so RED, Red, and red all represent the same value
...
def parse_colour_name(strdata: str) -> pygame.Color:
    """| Parse Colour name string into its corresponding colour value | As of the writing of these documentations,
    all colour names defined are from the CSS colours given by all major browsers, which can be found here:
    https://w3schools.sinsixx.com/css/css_colornames.asp.htm | All colour names are not case-sensitive, so RED, Red,
    and red all represent the same value

    :param strdata: The colour name to parse :type strdata: str
    :returns: A pygame Color from the colour name given by strdata :rtype: pygame.Color
    :raises KeyError: If the colour corresponding to the strdata passed in could not be found ( always call
                      is_valid_colour_name first to check )
    """
...
def parse_colour_string(strdata: str) -> Optional[pygame.Color]:
    ...
    :type strdata: str
    :return: A Pygame Color Object from the colour data parsed from strdata or None if the strdata is an invalid
             colour string
...
def valid_enclosing_glyphs(strdata: str) -> bool:
    ...
    :type strdata: str
    :returns: A boolean determining whether each opening parenthesis has a closing parenthesis and each closing
              parenthesis has an open parenthesis
    :rtype: bool
    """
    ...
    for ch in strdata:
        if ch in glyphs.values():
            ...
            if opening_stack and opening_stack[-1] == glyphs[ch]:
                opening_stack.pop()
                continue
            return False
...
def get_commas_outside_enclosing_glyphs(strdata: str) -> List[int]:
    """In the colour_parser module, This function is used to determine where to split gradient strings in order to get
       the full list of colours and the final degrees
        Developer Notes:
            - Used to determine which top level commas should be used to separate gradients
            - Used with the assumption that colour values themselves do not have glyphs that are left opened
            - Used with the assumption that top level commas used to separate gradients are not within any sort of
              enclosing glyph
...
def may_be_gradient_string(strdata: str) -> bool:
    """Determine if a string should even be considered for validation as a gradient
        Developer Notes:
            - Determines this by determining if there are 2 or 3 commas outside any enclosing glyph and if the
              enclosing glyphs in the colour string are validly closed and opened.
...
def parse_gradient_string(strdata: str) -> Optional[ColourGradient]:
    ...
    :param strdata: the gradient string to validate
    :type strdata: str
    :returns: A pygame_gui.core.colour_gradient.ColourGradient object if strdata is a valid gradient string, otherwise
              None
    :rtype: bool or None
    """
...
def parse_colour_or_gradient_string(strdata: str) -> Optional[Union[pygame.Color, ColourGradient]]:
    ...
    |
    | The documentation for what counts as a 'Valid' colour and gradient string, including the supported colour
    | formats by pygame_gui, can be found in the Theme Guide of the pygame_gui documentation at
    | https://pygame-gui.readthedocs.io/en/v_065/theme_guide.html
