...
class RoundedRectangleShape(DrawableShape):
    ...
    def full_rebuild_on_size_change(self):
        ...
        if self.shadow_width > 0:
            ...
            for i, corner_radius in enumerate(corner_radii):
                ...
                if corner_radius > min(
                    self.click_area_shape.width / 2, self.click_area_shape.height / 2
                ):
                    corner_radius = int(
                        min(
                            self.click_area_shape.width / 2,
                ...
                if corner_radius < 0:
                    corner_radius = 0
                    warnings.warn(
                        f"Clamping shape_corner_radius of: {str(old_radius)}, to: 0"
                    )
            ...
            shadow = self.ui_manager.get_shadow(
                ...
                "rectangle",
                corner_radius=[
                    self.shape_corner_radius[0] + self.shadow_width,
    ...
    def collide_point(
        ...
        if self.click_area_shape.collidepoint(point[0], point[1]):
            ...
            max_left_corner_radius = max(
                self.shape_corner_radius[0], self.shape_corner_radius[2]
            )
            max_right_corner_radius = max(
                self.shape_corner_radius[1], self.shape_corner_radius[3]
            )
            center_rect = pygame.Rect(
                self.click_area_shape.x + max_left_corner_radius,
                self.click_area_shape.y,
                self.click_area_shape.width
                - (max_left_corner_radius + max_right_corner_radius),
                self.click_area_shape.height,
            )

            edge_rect_left = pygame.Rect(
                self.click_area_shape.x,
            ...
            edge_rect_right = pygame.Rect(
                self.click_area_shape.x
                + self.click_area_shape.width
                - max_right_corner_radius,
                self.click_area_shape.y + self.shape_corner_radius[1],
                max_right_corner_radius,
                self.click_area_shape.height
                - (self.shape_corner_radius[1] + self.shape_corner_radius[3]),
            )
            ...
            if center_rect.collidepoint(point[0], point[1]):
                collided = True
            elif edge_rect_left.collidepoint(
                point[0], point[1]
            ) or edge_rect_right.collidepoint(point[0], point[1]):
                collided = True
            else:
                point_vec = Vector2(point)
                corner_centers = [
                    Vector2(
                        self.click_area_shape.x + self.shape_corner_radius[0],
                        self.click_area_shape.y + self.shape_corner_radius[0],
                    ),
                    Vector2(
                        self.click_area_shape.x
                    ...
                    Vector2(
                        self.click_area_shape.x + self.shape_corner_radius[2],
                        self.click_area_shape.y
                        + self.click_area_shape.height
                ...
                corner_center_distances = [
                    point_vec.distance_to(center) for center in corner_centers
                ]
    ...
    def set_dimensions(
        ...
        if dimensions[0] <= 0 or dimensions[1] <= 0:
            ...
            if self.shadow_width > 0:
                ...
                quick_surf = pygame.surface.Surface(
                    self.containing_rect.size, flags=pygame.SRCALPHA, depth=32
                )
                quick_surf.fill(pygame.Color("#00000000"))
            if isinstance(self.theming["normal_bg"], ColourGradient):
                grad_surf = pygame.surface.Surface(
                    self.click_area_shape.size, flags=pygame.SRCALPHA, depth=32
                )
    ...
    def redraw_state(self, state_str: str, add_text: bool = True):
        ...
        if self.containing_rect.width <= 0 or self.containing_rect.height <= 0:
            ...
            if found_shape is not None:
                ...
                dimension_scale = min(
                    self.background_rect.width / max(self.border_rect.width, 1),
    ...
    def _redraw_filled_bar(
        ...
        filled_bar_width = int(
            self.background_rect.width * self.theming["filled_bar_width_percentage"]
        )
        ...
        unfilled_bar_rect = pygame.Rect(
            (filled_bar_width, 0), (unfilled_bar_width, self.background_rect.height)
        )
    ...
    def clear_and_create_shape_surface(
        ...
        for i, corner_radius in enumerate(corner_radii):
            if corner_radius > min(rect.width / 2, rect.height / 2):
                large_corner_radii[i] = (
                    max(int(min(rect.width / 2, rect.height / 2)), 0) * aa_amount
                )
            else:
        ...
        if self.temp_additive_shape is None:
            large_shape_surface = pygame.surface.Surface(
                (rect.width, rect.height), flags=pygame.SRCALPHA, depth=32
            )
            ...
            RoundedRectangleShape.draw_colourless_rounded_rectangle(
                large_corner_radii, large_shape_surface
            )
            self.temp_additive_shape = large_shape_surface.copy()
        else:
            large_shape_surface = pygame.transform.scale(
                self.temp_additive_shape, (rect.width, rect.height)
            )
        ...
        if clear:
            ...
            if subtract_rect.width > 0 and subtract_rect.height > 0:
                if self.temp_subtractive_shape is None:
                    large_sub_surface = self.create_subtract_surface(
                        subtract_rect.size, large_corner_radii, aa_amount
                    )
                    self.temp_subtractive_shape = large_sub_surface
                else:
                    large_sub_surface = pygame.transform.scale(
                        self.temp_subtractive_shape, subtract_rect.size
                    )
    ...
    def create_subtract_surface(
        self, subtract_size: Tuple[int, int], corner_radii: List[int], aa_amount: int
    ):
        """
        ...
        if self.temp_subtractive_shape is not None:
            return pygame.transform.scale(self.temp_subtractive_shape, subtract_size)
        # for the subtract surface we want to blend in all RGBA channels to clear
        # correctly for our new shape
        self.temp_subtractive_shape = pygame.surface.Surface(
            subtract_size, flags=pygame.SRCALPHA, depth=32
        )
