...
class EllipseDrawableShape(DrawableShape):
    ...
    def __init__(
        ...
        *,
        ...
        self.ellipse_half_diameters = (
            0.5 * containing_rect.width,
            0.5 * containing_rect.height,
        )
    ...
    def full_rebuild_on_size_change(self):
        ...
        if self.shadow_width > 0:
            ...
            self.base_surface = self.ui_manager.get_shadow(
                self.containing_rect.size, self.shadow_width, "ellipse"
            )
        else:
    ...
    def collide_point(
        ...
        """
        x_val = ((point[0] - self.ellipse_center[0]) ** 2) / (
            self.ellipse_half_diameters[0] ** 2
        )
        y_val = ((point[1] - self.ellipse_center[1]) ** 2) / (
            self.ellipse_half_diameters[1] ** 2
        )
    ...
    def set_dimensions(
        ...
        self.ellipse_half_diameters = (
            0.5 * self.containing_rect.width,
        ...
        if self.containing_rect.width <= 0 or self.containing_rect.height <= 0:
            ...
            if found_shape is not None:
                ...
                if isinstance(self.theming[bg_colour_state_str], ColourGradient):
                    ...
                    apply_colour_to_surface(
                        self.theming[bg_colour_state_str], shape_surface
                    )
                ...
                pygame.draw.ellipse(
                    sub_surface, pygame.Color("#FFFFFFFF"), sub_surface.get_rect()
                )
                ...
                self.states[state_str].surface.blit(
                    small_sub,
                    pygame.Rect(
                        (self.shadow_width, self.shadow_width), sub_surface.get_size()
                    ),
    ...
    def clear_and_create_shape_surface(
        ...
        if clear:
            ...
            surface.blit(
                large_sub_surface, subtract_rect, special_flags=pygame.BLEND_RGBA_SUB
            )
