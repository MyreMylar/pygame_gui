...
class ShadowGenerator:
    ...
    appearance unsatisfactory then it is possible to create more that are closer to the size of the
    ...
    def create_shadow_corners(
        self, shadow_width_param: int, corner_radii: List[int], aa_amount=4
        ...
        tl_corner = self._create_single_corner(
            aa_amount, corner_radii[0], shadow_width_param
        )
        tr_corner = flip(
            self._create_single_corner(aa_amount, corner_radii[1], shadow_width_param),
            flip_x=True,
            flip_y=False,
        )
        bl_corner = flip(
            self._create_single_corner(aa_amount, corner_radii[2], shadow_width_param),
            flip_x=False,
            flip_y=True,
        )
        br_corner = flip(
            self._create_single_corner(aa_amount, corner_radii[3], shadow_width_param),
            flip_x=True,
            flip_y=True,
        )
        ...
        self.preloaded_shadow_corners[
            (str(shadow_width_param) + "x" + str(corner_radii))
        ] = corners_and_edges
    ...
    def _create_single_corner(aa_amount, corner_radius_param, shadow_width_param):
        ...
        final_corner_surface = smoothscale(
            final_corner_surface, (corner_radius_param, corner_radius_param)
        )
        ...
        if sub_radius > 0:
            ...
            circle_center = corner_rect.bottomright

            pygame.draw.circle(
                corner_sub_surface, pygame.Color("#FFFFFFFF"), circle_center, sub_radius
            )
    ...
    def create_new_rectangle_shadow(
        ...
        corner_index_id = f"{shadow_width_param}x{corner_radii}"
        if corner_index_id in self.preloaded_shadow_corners:
            edges_and_corners = self.preloaded_shadow_corners[corner_index_id]
        else:
            edges_and_corners = self.create_shadow_corners(
                shadow_width_param, corner_radii
            )
        ...
        final_surface.blit(
            edges_and_corners["bottom_left"], (0, height - corner_radii[2])
        )
        final_surface.blit(
        ...
        if width - (corner_radii[0] + corner_radii[1]) > 0:
            top_edge = pygame.transform.scale(
                ...
                (width - (corner_radii[0] + corner_radii[1]), shadow_width_param),
            )
        ...
        if width - (corner_radii[2] + corner_radii[3]) > 0:
            bottom_edge = pygame.transform.scale(
                ...
                (width - (corner_radii[2] + corner_radii[3]), shadow_width_param),
            )
            final_surface.blit(
                bottom_edge, (corner_radii[2], height - shadow_width_param)
            )

        if height - (corner_radii[0] + corner_radii[2]) > 0:
            left_edge = pygame.transform.scale(
                edges_and_corners["left"],
                (shadow_width_param, height - (corner_radii[0] + corner_radii[2])),
            )
        ...
        if height - (corner_radii[1] + corner_radii[3]) > 0:
            right_edge = pygame.transform.scale(
                ...
                (shadow_width_param, height - (corner_radii[1] + corner_radii[3])),
            )
            final_surface.blit(
                right_edge, (width - shadow_width_param, corner_radii[1])
            )
    ...
    def create_new_ellipse_shadow(
        self, width: int, height: int, shadow_width_param: int, aa_amount: int = 4
        ...
        alpha_increment = max(1, 20 // shadow_width_param)
        ...
        for i in range(shadow_width_param):
            if shadow_width > 0 and shadow_height > 0:
                ...
                pygame.draw.ellipse(
                    shadow_surface, pygame.Color(0, 0, 0, shadow_alpha), shadow_rect
                )
        ...
        self.created_ellipse_shadows[f"{width}x{height}x{shadow_width_param}"] = (
            final_surface
        )
        return final_surface

    def find_closest_shadow_scale_to_size(
        self,
        size: Tuple[int, int],
        shadow_width: int = 2,
        shape: str = "rectangle",
        ...
        if shape == "ellipse":
            for key in self.created_ellipse_shadows:
                ...
                total_diff = (
                    width_diff + height_diff + (abs(shadow_size - shadow_width) * 50)
                )
            ...
            if closest_key is not None:
                return pygame.transform.smoothscale(
                    self.created_ellipse_shadows[closest_key], size
                )
            else:
