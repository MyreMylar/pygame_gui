...
class TestSurfaceCache:
    ...
    def test_add_surface_to_long_term_cache_too_large(self, _init_pygame):
        ...
        with pytest.warns(UserWarning, match="Unable to cache surfaces larger than"):
            cache.add_surface_to_long_term_cache(pygame.Surface((2048, 2048)),
                                                 string_id="test_surface")

    def test_add_surface_to_long_term_cache(self, _init_pygame):
        cache = SurfaceCache()
        cache.add_surface_to_long_term_cache(pygame.Surface((256, 256)),
                                             string_id="test_surface_1")
        cache.add_surface_to_long_term_cache(pygame.Surface((512, 256)),
                                             string_id="test_surface_2")
        cache.add_surface_to_long_term_cache(pygame.Surface((256, 512)),
                                             string_id="test_surface_3")
        cache.add_surface_to_long_term_cache(pygame.Surface((256, 128)),
                                             string_id="test_surface_4")
        cache.add_surface_to_long_term_cache(pygame.Surface((128, 256)),
