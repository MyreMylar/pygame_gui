...
class TestSurfaceCache:
    ...
    def test_add_surface_to_long_term_cache_too_large(
        ...
        with pytest.warns(UserWarning, match="Unable to cache surfaces larger than"):
            cache.add_surface_to_long_term_cache(
                {"surface": pygame.Surface((2048, 2048)), "uses": 1},
                string_id="test_surface",
            )

    def test_add_surface_to_long_term_cache(
        self, _init_pygame, _display_surface_return_none
    ):
        cache = SurfaceCache()
        cache.add_surface_to_long_term_cache(
            {"surface": pygame.Surface((256, 256)), "uses": 1},
            string_id="test_surface_1",
        )
        cache.add_surface_to_long_term_cache(
            {"surface": pygame.Surface((512, 256)), "uses": 1},
            string_id="test_surface_2",
        )
        cache.add_surface_to_long_term_cache(
            {"surface": pygame.Surface((256, 512)), "uses": 1},
            string_id="test_surface_3",
        )
        cache.add_surface_to_long_term_cache(
            {"surface": pygame.Surface((256, 128)), "uses": 1},
            string_id="test_surface_4",
        )
        cache.add_surface_to_long_term_cache(
            {"surface": pygame.Surface((128, 256)), "uses": 1},
        ...
        cache.add_surface_to_long_term_cache(
            {"surface": pygame.Surface((128, 128)), "uses": 1},
            string_id="test_surface_7",
        )
        cache.add_surface_to_long_term_cache(
            {"surface": pygame.Surface((640, 64)), "uses": 1},
            string_id="test_surface_8",
        )
        cache.add_surface_to_long_term_cache(
            {"surface": pygame.Surface((640, 32)), "uses": 1},
            string_id="test_surface_9",
        )
        cache.add_surface_to_long_term_cache(
            {"surface": pygame.Surface((784, 784)), "uses": 1},
    ...
    def test_remove_user_from_cache_item(
        self, _init_pygame, _display_surface_return_none
    ):
        cache = SurfaceCache()
        cache.add_surface_to_cache(pygame.Surface((64, 64)), "doop")
    ...
    def test_remove_user_and_request_clean_up_of_cached_item(
        self, _init_pygame, _display_surface_return_none
    ):
        cache = SurfaceCache()
        cache.add_surface_to_cache(pygame.Surface((64, 64)), "doop")
    ...
    def test_build_cache_id(self, _init_pygame, _display_surface_return_none):
        cache_id = SurfaceCache.build_cache_id(
            shape="rectangle",
            size=(64, 64),
            shadow_width=1,
        ...
        assert (
            cache_id == "rectangle_64_64_1_1_1_1_1_5_5_5_5_255_0_0_255_100_200_100_180"
        )
