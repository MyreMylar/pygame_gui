...
from pygame_gui.core.ui_appearance_theme import UIAppearanceTheme, UIThemeValidator, UIThemeValidationError
...
class TestUIAppearanceTheme:
    ...
    def test_theme_validator_valid_theme(self, _init_pygame, _display_surface_return_none):
    ...
    def test_theme_validator_invalid_colors(self, _init_pygame, _display_surface_return_none):
        ...
        assert any("Color values must be numbers between 0-255" in error for error in errors)
        assert any("Color must be string or tuple, got int" in error for error in errors)

    def test_theme_validator_invalid_misc(self, _init_pygame, _display_surface_return_none):
        ...
        assert any("Must be one of ['rectangle', 'rounded_rectangle', 'ellipse']" in error for error in errors)
        assert any("Boolean field must be '0', '1', 'true', or 'false'" in error for error in errors)

    def test_theme_validator_invalid_font(self, _init_pygame, _display_surface_return_none):
    ...
    def test_theme_validator_invalid_images(self, _init_pygame, _display_surface_return_none):
        ...
        errors = UIThemeValidator.validate_theme_file(invalid_theme)
        assert len(errors) == 2
        assert any("Must have either 'path' or both 'package' and 'resource'" in error for error in errors)
        assert any("Must have 4 comma-separated values, got 3" in error for error in errors)

    def test_theme_validator_invalid_element_type(self, _init_pygame, _display_surface_return_none):
    ...
    def test_theme_validator_color_validation_methods(self, _init_pygame, _display_surface_return_none):
    ...
    def test_theme_validator_misc_validation_methods(self, _init_pygame, _display_surface_return_none):
    ...
    def test_theme_validator_font_validation_methods(self, _init_pygame, _display_surface_return_none):
    ...
    def test_theme_validator_image_validation_methods(self, _init_pygame, _display_surface_return_none):
    ...
    def test_appearance_theme_validate_theme_data(self, _init_pygame, _display_surface_return_none):
    ...
    def test_appearance_theme_validation_integration(self, _init_pygame, _display_surface_return_none):
    ...
    def test_theme_validator_text_shadow_offset_validation(self, _init_pygame, _display_surface_return_none):
        ...
        valid_themes = [
            ...
            {"button": {"misc": {"text_shadow_offset": " -1 , 5 "}}},  # String with spaces
        ...
        invalid_themes = [
            ({"button": {"misc": {"text_shadow_offset": "5"}}}, "Must have 2 comma-separated values"),
            ({"button": {"misc": {"text_shadow_offset": "1,2,3"}}}, "Must have 2 comma-separated values"),
            ({"button": {"misc": {"text_shadow_offset": "abc,def"}}}, "expected 'x,y' with numeric values"),
            ({"button": {"misc": {"text_shadow_offset": (1, 2, 3)}}}, "Must have 2 values"),
            ({"button": {"misc": {"text_shadow_offset": ('a', 'b')}}}, "Values must be numeric"),
            ({"button": {"misc": {"text_shadow_offset": 5}}}, "Must be string 'x,y' or tuple/list"),
            ({"button": {"misc": {"text_shadow_offset": (100, 0)}}}, "X value 100.0 must be between -50 and 50"),
            ({"button": {"misc": {"text_shadow_offset": (0, -100)}}}, "Y value -100.0 must be between -50 and 50"),
        ]
        
        for theme_data, expected_message in invalid_themes:
            errors = UIThemeValidator.validate_theme_file(theme_data)
            assert len(errors) >= 1, f"Invalid theme should have errors: {theme_data}"
            assert expected_message in str(errors), f"Expected message '{expected_message}' not found in errors: {errors}"

    def test_theme_validator_shape_corner_radius_validation(self, _init_pygame, _display_surface_return_none):
        ...
        valid_themes = [
            ...
            {"button": {"misc": {"shape_corner_radius": " 1 , 2 , 3 , 4 "}}},  # 4-corner string with spaces
            {"button": {"misc": {"shape_corner_radius": (5, 10, 15, 20)}}},  # 4-corner tuple
            {"button": {"misc": {"shape_corner_radius": [0, 5, 10, 15]}}},  # 4-corner list
            {"button": {"misc": {"shape_corner_radius": [25]}}},  # Single value in list
            {"button": {"misc": {"shape_corner_radius": (0, 100, 0, 100)}}},  # Boundary values
        ]
        ...
        # Test invalid formats
        invalid_themes = [
            ({"button": {"misc": {"shape_corner_radius": "1,2"}}}, "Must have 1 value (all corners) or 4 comma-separated values"),
            ({"button": {"misc": {"shape_corner_radius": "1,2,3"}}}, "Must have 1 value (all corners) or 4 comma-separated values"),
            ({"button": {"misc": {"shape_corner_radius": "abc,def,ghi,jkl"}}}, "corner value must be numeric"),
            ({"button": {"misc": {"shape_corner_radius": (1, 2, 3)}}}, "Must have 1 value (all corners) or 4 values"),
            ({"button": {"misc": {"shape_corner_radius": ('a', 'b', 'c', 'd')}}}, "corner value must be numeric"),
            ({"button": {"misc": {"shape_corner_radius": {"radius": 5}}}}, "Must be number, string, or list/tuple"),
            ({"button": {"misc": {"shape_corner_radius": 150}}}, "Value 150 must be between 0 and 100"),
            ({"button": {"misc": {"shape_corner_radius": -5}}}, "Value -5 must be between 0 and 100"),
            ({"button": {"misc": {"shape_corner_radius": (5, 150, 10, 20)}}}, "top-right corner value 150.0 must be between 0 and 100"),
        ]
