...
from pygame_gui.core.drawable_shapes.drawable_shape import (
    DrawableShape,
    DrawableShapeState,
...
class TestDrawableShapeState:
    ...
    def test_get_surface(
        ...
        states = {"normal": normal_state, "selected": selected_state}
        assert isinstance(normal_state.get_surface(), pygame.Surface)

        normal_state.transition = DrawableStateTransition(
            states=states,
...
class TestDrawableShape:
    def test_creation(self, _init_pygame, default_ui_manager: UIManager):
        ...
        DrawableShape(
            containing_rect=pygame.Rect(0, 0, 0, 0),
            theming_parameters={},
            states=["normal"],
            manager=default_ui_manager,
        )
        ...
        with pytest.raises(
            NotImplementedError,
            match="No 'normal' state id supplied for drawable shape",
        ):
            DrawableShape(
                containing_rect=pygame.Rect(0, 0, 100, 100),
                theming_parameters={},
    ...
    def test_set_active_state_with_transitions(
        self, _init_pygame, default_ui_manager: UIManager
    ):
        shape = DrawableShape(
            containing_rect=pygame.Rect(0, 0, 100, 100),
            theming_parameters={
                "transitions": {("normal", "hovered"): 0.5, ("hovered", "normal"): 0.5}
            },
            states=["normal", "hovered"],
            manager=default_ui_manager,
        )
    ...
    def test_full_rebuild_on_size_change(
        self, _init_pygame, default_ui_manager: UIManager
    ):
        shape = DrawableShape(
            containing_rect=pygame.Rect(0, 0, 100, 100),
        ...
        assert (
            shape.full_rebuild_countdown
            == shape.time_until_full_rebuild_after_changing_size
        )
    ...
    def test_apply_colour_to_surface(
        self, _init_pygame, default_ui_manager: UIManager, default_display_surface
    ):
        DrawableShape(
            containing_rect=pygame.Rect(0, 0, 100, 100),
            theming_parameters={},
        ...
        apply_colour_to_surface(
            pygame.Color(150, 100, 150, 255), test_surface_2, pygame.Rect(0, 0, 25, 50)
        )
    ...
    def test_rebuild_images_and_text(
        ...
        shape = DrawableShape(
            ...
            theming_parameters={
                ...
                "text_shadow": (0, 0, 0, pygame.Color("#000000"), False),
            },
            states=["normal"],
            manager=default_ui_manager,
        )
        ...
        shape.finalise_images_and_text(
