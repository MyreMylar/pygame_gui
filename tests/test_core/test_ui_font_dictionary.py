...
class TestUIFontDictionary:
    def test_creation(self, _init_pygame, _display_surface_return_none):
        font_dictionary = UIFontDictionary(
            BlockingThreadedResourceLoader(), locale="en"
        )
        assert font_dictionary.loaded_fonts is not None

    def test_load_default_font_from_strings(
        ...
        font_dictionary.preload_font(
            font_size=14, font_name="noto_sans", bold=True, italic=True
        )
    ...
    def test_load_bad_default_font(self, _init_pygame, _display_surface_return_none):
        font_dictionary = UIFontDictionary(
            BlockingThreadedResourceLoader(), locale="derp"
        )
        bad_font = DefaultFontData(
            14,
            "bad-font",
            "regular",
            "not_a_real_font.ttf",
    ...
    def test_find_font_unloaded_style_bold(
        self, _init_pygame, _display_surface_return_none
    ):
        font_dictionary = UIFontDictionary(
            BlockingThreadedResourceLoader(), locale="en"
    ...
    def test_find_font_unloaded_style_italic(
        self, _init_pygame, _display_surface_return_none
    ):
        font_dictionary = UIFontDictionary(
            BlockingThreadedResourceLoader(), locale="en"
    ...
    def test_find_font_unloaded_style_bold_italic(
        self, _init_pygame, _display_surface_return_none
    ):
        font_dictionary = UIFontDictionary(
            BlockingThreadedResourceLoader(), locale="en"
        ...
        with pytest.warns(UserWarning, match="Finding font with id"):
            font_dictionary.find_font(
                font_size=20, font_name="noto_sans", bold=True, italic=True
            )
    ...
    def test_find_font_unloaded(self, _init_pygame, _display_surface_return_none):
        ...
        font_dictionary_2 = UIFontDictionary(
            BlockingThreadedResourceLoader(), locale="en"
        )
        font_dictionary_2.find_font(
            font_size=20, font_name="sans", bold=True, italic=True
        )

        font_dictionary_3 = UIFontDictionary(
            BlockingThreadedResourceLoader(), locale="en"
        )
        font_dictionary_3.find_font(
            font_size=20, font_name="sans", bold=True, italic=False
        )

        font_dictionary_4 = UIFontDictionary(
            BlockingThreadedResourceLoader(), locale="en"
        )
        font_dictionary_4.find_font(
            font_size=20, font_name="sans", bold=False, italic=True
        )
    ...
    def test_create_font_id(self, _init_pygame, _display_surface_return_none):
        ...
        font_id_1 = font_dictionary.create_font_id(
            font_size=10, font_name="bob", bold=False, italic=False
        )
        assert font_id_1 == "bob_regular_aa_10"

        font_id_1 = font_dictionary.create_font_id(
            font_size=10, font_name="bob", bold=True, italic=False
        )
        assert font_id_1 == "bob_bold_aa_10"

        font_id_1 = font_dictionary.create_font_id(
            font_size=10, font_name="bob", bold=False, italic=True
        )
        assert font_id_1 == "bob_italic_aa_10"

        font_id_1 = font_dictionary.create_font_id(
            font_size=10, font_name="bob", bold=True, italic=True
        )
        assert font_id_1 == "bob_bold_italic_aa_10"

        with pytest.warns(UserWarning, match="Font size less than or equal to 0"):
            font_dictionary.create_font_id(
                font_size=-50, font_name="bob", bold=True, italic=True
            )
    ...
    def test_preload_no_paths(self, _init_pygame, _display_surface_return_none):
        ...
        font_dictionary_2.preload_font(
            font_size=20, font_name="sans", bold=True, italic=True
        )
        ...
        font_dictionary_3.preload_font(
            font_size=20, font_name="sans", bold=True, italic=False
        )
        ...
        font_dictionary_4.preload_font(
            font_size=20, font_name="sans", bold=False, italic=True
        )
    ...
    def test_preload_bad_paths(self, _init_pygame, _display_surface_return_none):
        ...
        with pytest.warns(UserWarning, match="Unable to load resource with path"):
            font_dictionary.preload_font(
                font_name="arial", font_size=14, bold=True, italic=True
            )
    ...
    def test_print_unused_loaded_fonts(
        self, _init_pygame, _display_surface_return_none, capsys
    ):
        loader = BlockingThreadedResourceLoader()
        font_dictionary = UIFontDictionary(loader, locale="en")
        font_dictionary.add_font_path(
    ...
    def test_convert_html_size_to_point_size_invalid(
        self, _init_pygame, _display_surface_return_none
    ):
        font_dictionary = UIFontDictionary(
            BlockingThreadedResourceLoader(), locale="en"
        ...
        assert (
            font_dictionary.convert_html_to_point_size(9000)
            == font_dictionary.default_font.size
        )

    def test_convert_html_size_to_point_size_valid(
        self, _init_pygame, _display_surface_return_none
    ):
        font_dictionary = UIFontDictionary(
            BlockingThreadedResourceLoader(), locale="en"
