...
class TestUIColourChannelEditor:
    def test_creation(
        ...
        UIColourChannelEditor(
            relative_rect=pygame.Rect(0, 0, 150, 29),
    ...
    def test_text_entry_finished(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        channel_editor = UIColourChannelEditor(
            relative_rect=pygame.Rect(0, 0, 150, 29),
            manager=default_ui_manager,
            name="H:",
            channel_index=0,
        ...
        default_ui_manager.process_events(
            pygame.event.Event(
                ...
                {"button": pygame.BUTTON_LEFT, "pos": channel_editor.entry.rect.center},
            )
        )
        default_ui_manager.process_events(
            pygame.event.Event(
        ...
        with pytest.warns(
            UserWarning,
            match="Tried to set text string with "
        ...
        channel_editor.entry.process_event(
            pygame.event.Event(pygame.KEYDOWN, {"key": pygame.K_RETURN})
        )
    ...
    def test_slider_moved_finished(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        channel_editor = UIColourChannelEditor(
            relative_rect=pygame.Rect(0, 0, 150, 29),
...
class TestUIColourPickerDialog:
    def test_creation(
        ...
        UIColourPickerDialog(
            rect=pygame.Rect(100, 100, 400, 400), manager=default_ui_manager
        )
    ...
    def test_create_too_small(
        ...
        with pytest.warns(UserWarning, match="Initial size"):
            UIColourPickerDialog(
                rect=pygame.Rect(100, 100, 50, 50), manager=default_ui_manager
            )
    ...
    def test_2d_slider_moved(
        ...
        colour_picker.process_event(
            pygame.event.Event(
                pygame_gui.UI_2D_SLIDER_MOVED,
                {"ui_element": colour_picker.colour_2d_slider, "value": (50, 50)},
            )
        )
    ...
    def test_press_ok_button(
        ...
        for event in pygame.event.get():
            ...
            if (
                ...
                and event.ui_element == colour_picker
            ):
    ...
    def test_mess_with_colour_channel_event(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        colour_picker = UIColourPickerDialog(
            rect=pygame.Rect(100, 100, 400, 400),
        ...
        for event in pygame.event.get():
            if (
                event.type == pygame_gui.UI_COLOUR_PICKER_COLOUR_CHANNEL_CHANGED
                and event.ui_element == colour_picker.red_channel
            ):
                confirm_event_fired = True
        ...
        for event in pygame.event.get():
            if (
                event.type == pygame_gui.UI_COLOUR_PICKER_COLOUR_CHANNEL_CHANGED
                and event.ui_element == colour_picker.hue_channel
            ):
                confirm_event_fired = True
    ...
    def test_update_saturation_value_square(
        ...
        colour_picker.current_colour.hsva = (
            colour_picker.hue_channel.current_value,
    ...
    def test_update_current_colour_image(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        colour_picker = UIColourPickerDialog(
            rect=pygame.Rect(100, 100, 400, 400),
        ...
        pixel_colour = restore_premul_col(
            pixel_colour
        )  # this is going to be slightly inaccurate
    ...
    def test_changed_rgb_update_hsv(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        colour_picker = UIColourPickerDialog(
            rect=pygame.Rect(100, 100, 400, 400),
        ...
        colour_picker.current_colour = pygame.Color(
            colour_picker.red_channel.current_value,
    ...
    def test_changed_hsv_update_rgb(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        colour_picker = UIColourPickerDialog(
            rect=pygame.Rect(100, 100, 400, 400),
