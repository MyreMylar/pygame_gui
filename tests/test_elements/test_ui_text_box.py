...
class TestUITextBox:
    def test_creation(
        ...
        text_box_with_kwargs = UITextBox(
    ...
    def test_create_too_narrow_textbox_for_font(
        ...
        with pytest.warns(
            UserWarning,
            match="Unable to split word into chunks because text box is too narrow",
        ):
            with pytest.raises(RuntimeError, match="Row longer than layout"):
                text_box = UITextBox(
                    html_text="la la LA LA LAL LAL ALALA"
    ...
    def test_update_without_scrollbar(
        ...
        text_box = UITextBox(
            html_text="<a href=None>lalaLAlalala</a>",
    ...
    def test_set_active_effect_typing(
        self,
        _init_pygame: None,
        default_ui_manager: UIManager,
        _display_surface_return_none: None,
        ...
        text_box.update(5.0)
        assert isinstance(
            text_box.active_text_effect, pygame_gui.core.text.TypingAppearEffect
        )

        # try setting a nonsense effect on a chunk
        with pytest.warns(
            UserWarning, match="Unsupported effect name: blork for text chunk"
        ):
            text_box.set_active_effect(
                effect_type=UITextEffectType("blork"), effect_tag="test"
            )
    ...
    def test_set_active_effect_with_word_split(
        ...
        htm_text_block_2 = UITextBox(
            "<font face=noto_sans size=2 color=#000000>"
    ...
    def test_process_event_mouse_buttons_no_scrollbar(
        ...
        text_box = UITextBox(
            html_text="<a href=none>alalaadads<a/>",
            relative_rect=pygame.Rect(0, 0, 150, 100),
            manager=default_ui_manager,
        )
    ...
    def test_rebuild_from_theme_data_non_default(
        ...
        text_box = UITextBox(
            html_text="<font color=#FF0000 face=noto_sans>Some "
    ...
    def test_rebuild_from_theme_data_bad_values(
        ...
        text_box = UITextBox(
            html_text="<font color=#FF0000 face=noto_sans>Some text in a <b>bold box</b> using "
            "colours and <i>styles</i>.</font>",
            relative_rect=pygame.Rect(100, 100, 200, 300),
            manager=manager,
        )
    ...
    def test_create_very_short_text_box(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        x = 640
        y = 360
    ...
    def test_show_without_scrollbar(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        x = 640
        y = 360
    ...
    def test_hide_without_scrollbar(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        x = 640
        y = 360
