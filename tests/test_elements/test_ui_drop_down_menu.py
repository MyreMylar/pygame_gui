...
class TestUIDropDownMenu:
    ...
    def test_removal(
        ...
        menu = UIDropDownMenu(
            options_list=["cheese", "eggs", "flour", "sugar"],
            starting_option="eggs",
            relative_rect=pygame.Rect(100, 100, 200, 30),
            manager=default_ui_manager,
        )
    ...
    def test_update_state_transition(
        ...
        menu.current_state.should_transition = True
        menu.update(0.01)
        menu.current_state.options_selection_list.item_list[0][
            "button_element"
        ].pressed = True
        default_ui_manager.update(0.01)
        assert menu.image is not None

    def test_update_closed_state_close_button(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        menu = UIDropDownMenu(
            options_list=["eggs", "flour", "sugar"],
    ...
    def test_update_open_state_finish(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        menu = UIDropDownMenu(
            options_list=["eggs", "flour", "sugar"],
    ...
    def test_rebuild_from_theme_data_non_default(
        ...
        manager = UIManager(
            (800, 600),
            os.path.join(
                "tests", "data", "themes", "ui_drop_down_menu_non_default.json"
            ),
        )
        ...
        menu.current_state.should_transition = True
        menu.update(0.01)
        manager.ui_theme.ui_element_misc_data["drop_down_menu"]["expand_direction"] = (
            "down"
        )
        ...
        item_list_height = 15
        assert menu.menu_states["expanded"].options_list_height == (
            3 * item_list_height + 2 * border_and_shadow_width
        )
        assert menu.image is not None
    ...
    def test_rebuild_from_theme_data_bad_values(
        ...
        manager = UIManager(
            (800, 600),
            os.path.join(
                "tests", "data", "themes", "ui_drop_down_menu_bad_values.json"
            ),
        )
    ...
    @pytest.mark.filterwarnings("ignore:Image data validation")
    @pytest.mark.filterwarnings("ignore:Invalid border_width value")
    def test_rebuild_from_theme_data_bad_values_live(
        self, _init_pygame, _display_surface_return_none
    ):
        ...
        # load new theme
        manager.ui_theme.load_theme(
            os.path.join("tests", "data", "themes", "ui_drop_down_menu_bad_values.json")
        )
    ...
    def test_set_relative_position(
        ...
        default_ui_manager.process_events(
            pygame.event.Event(
                ...
                {"button": pygame.BUTTON_LEFT, "pos": (260, 145)},
            )
        )

        assert (
            menu.rect.topleft == (250, 130)
            and menu.current_state.selected_option_button.held is True
        )
    ...
    def test_set_dimensions(
        ...
        assert menu.current_state.open_button.relative_rect.right == 300 - (
            menu.border_width["right"] + menu.shadow_width
        )

        assert menu.current_state.open_button.relative_rect.bottom == 50 - (
            menu.border_width["bottom"] + menu.shadow_width
        )
        ...
        default_ui_manager.process_events(
            pygame.event.Event(
                pygame.MOUSEBUTTONDOWN,
                {"button": pygame.BUTTON_LEFT, "pos": (390, 125)},
            )
        )
        # if we successfully clicked on the moved menu then this button should be True
        ...
        # try to click on the menu
        default_ui_manager.process_events(
            pygame.event.Event(pygame.MOUSEBUTTONDOWN, {"button": 1, "pos": (290, 115)})
        )
        # if we successfully clicked on the moved menu then this button should be True
        ...
        non_default_menu = UIDropDownMenu(
            options_list=["eggs", "flour", "sugar"],
            starting_option="eggs",
            relative_rect=pygame.Rect(100, 100, 200, 30),
            manager=manager,
        ...
        assert non_default_menu.current_state.open_button.relative_rect.right == 300 - (
            non_default_menu.border_width["right"] + non_default_menu.shadow_width
        )

        assert non_default_menu.current_state.open_button.relative_rect.bottom == 50 - (
            non_default_menu.border_width["bottom"] + non_default_menu.shadow_width
        )
    ...
    def test_cropping_size_of_drop_down(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        menu = UIDropDownMenu(
            options_list=[
        ...
        assert (
            menu.current_state.options_selection_list.rect.height == 200
        )  # cropped to fixed height

        test_container = UIContainer(
            relative_rect=pygame.Rect(100, 100, 300, 100), manager=default_ui_manager
        )
        menu = UIDropDownMenu(
            options_list=[
                "eggs",
        ...
        assert (
            menu.current_state.options_selection_list.rect.height == 63
        )  # cropped to container size by default
        ...
        test_container = UIContainer(
            relative_rect=pygame.Rect(100, 100, 300, 100), manager=manager
        )
        menu = UIDropDownMenu(
            options_list=[
                "eggs",
        ...
        assert (
            menu.current_state.options_selection_list.rect.height == 53
        )  # cropped to container size by default

    def test_select_option_from_drop_down(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        test_container = UIContainer(
        ...
        flour_button = (
            menu.current_state.options_selection_list.item_list_container.elements[1]
        )

        flour_button.process_event(
            pygame.event.Event(
                pygame.MOUSEBUTTONDOWN,
                {"button": pygame.BUTTON_LEFT, "pos": flour_button.rect.center},
            )
        )
    ...
    def test_disable(
        ...
        expand_button.process_event(
            pygame.event.Event(
                pygame.MOUSEBUTTONDOWN,
                {"button": pygame.BUTTON_LEFT, "pos": expand_button.rect.center},
            )
        )
    ...
    def test_hide_of_closed_dropdown(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        menu = UIDropDownMenu(
            options_list=["eggs", "flour", "sugar"],
    ...
    def test_hide_of_expanded_dropdown(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        menu = UIDropDownMenu(
            options_list=["eggs", "flour", "sugar"],
    ...
    def test_class_theming_id(self, _init_pygame, _display_surface_return_none):
        ...
        menu = UIDropDownMenu(
            ...
            relative_rect=pygame.Rect(10, 10, 200, 30),
            manager=manager,
            object_id=pygame_gui.core.ObjectID(object_id=None, class_id="@test_class"),
        )
