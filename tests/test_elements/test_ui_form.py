...
def get_questionnaire():
    ...
    questionnaire = {
        ...
        "Section Test:": {
            ...
            "UISelectionList Multiselect Test": UISelectionList(
                test_rect, ["item 1", "item 2"], allow_multi_select=True
            ),
            "UIDropDownMenu Test": UIDropDownMenu(
                ["item 1", "item 2"], "item 1", test_rect
            ),
        },
    }
...
class TestUIForm:
    ...
    def test_disable(
        ...
        default_ui_manager.process_events(
            pygame.event.Event(
                ...
                {"button": pygame.BUTTON_LEFT, "pos": element.rect.center},
            )
        )
        default_ui_manager.process_events(
            pygame.event.Event(
    ...
    def test_show_hide_rendering(
        ...
        container = UIForm(
            relative_rect=pygame.Rect(100, 100, 200, 100), manager=manager, visible=0
        )
        manager.update(0.01)
    ...
    def test_get_current_values(
        ...
        assert container.get_current_values() == {
            "Character Test:": "",
            "Short Text Test:": "",
            "Long Text Test:": "",
            "Integer Test:": 0,
        ...
        element.process_event(
            pygame.event.Event(pygame.TEXTINPUT, {"text": "a1.b2.c3"})
        )
        assert (
            container.get_current_values()["Decimal Test:"] == 0.0
        )  # Invalid input results in 0.0
        ...
        element = element.current_state.options_selection_list.item_list[-1][
            "button_element"
        ]
        default_ui_manager.process_events(
            pygame.event.Event(
    ...
    def test_rebuild_from_theme_data_non_default(
        ...
        manager = UIManager(
            (800, 600),
            os.path.join("tests", "data", "themes", "ui_2d_slider_non_default.json"),
        )
    ...
    def test_rebuild_from_theme_data_bad_values(
        ...
        manager = UIManager(
            (800, 600),
            os.path.join("tests", "data", "themes", "ui_2d_slider_bad_values.json"),
        )
    ...
    def test_type_checker(
        ...
        assert UIForm.type_checker(
            f"{w}short_text{w}({w}' a' \t\f'b' \t\f\"c\"{w},{w}required{w}={w}True{w}){w}"
        ) == ("short_text", {"default": " abc", "required": True})

        # Syntax + Character

        assert UIForm.type_checker("character") == ("character", {})
        assert UIForm.type_checker("character('a')") == ("character", {"default": "a"})
        assert UIForm.type_checker("character(default='a')") == (
            "character",
            {"default": "a"},
        )
        assert UIForm.type_checker("character('a',True)") == (
            "character",
            {"default": "a", "required": True},
        )
        assert UIForm.type_checker("character('a',False)") == (
            "character",
            {"default": "a", "required": False},
        )
        assert UIForm.type_checker("character('a',required=True)") == (
            "character",
            {"default": "a", "required": True},
        )
        assert UIForm.type_checker("character(default='a',required=False)") == (
            "character",
            {"default": "a", "required": False},
        )

        # Short Text
        assert UIForm.type_checker("short_text('abc', required=True)") == (
            "short_text",
            {"default": "abc", "required": True},
        )

        # Long Text
        assert UIForm.type_checker("long_text('abc', required=True)") == (
            "long_text",
            {"default": "abc", "required": True},
        )

        # Integer Text
        assert UIForm.type_checker("integer(1,required=True)") == (
            "integer",
            {"default": 1, "required": True},
        )

        # Decimal Text
        assert UIForm.type_checker("decimal(1,required=True)") == (
            "decimal",
            {"default": 1, "required": True},
        )
        assert UIForm.type_checker("decimal(1.0,required=False)") == (
            "decimal",
            {"default": 1.0, "required": False},
        )

        # Password Text
        assert UIForm.type_checker("password('abc', True)") == (
            "password",
            {"default": "abc", "required": True},
        )

        # Boolean Text
        assert UIForm.type_checker("boolean(True, True)") == (
            "boolean",
            {"default": True, "required": True},
        )
        assert UIForm.type_checker("boolean(False, False)") == (
            "boolean",
    ...
    @pytest.mark.filterwarnings("ignore:Font data validation")
    @pytest.mark.filterwarnings("ignore:Image data validation")
    def test_type_checker_bad_values(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        # General

        with pytest.raises(
            ValueError, match="Question type 'unknown' is not supported"
        ):
            UIForm.type_checker("unknown")

        with pytest.raises(
            SyntaxError,
            match="Parenthesis used without passing parameters for question",
        ):
            UIForm.type_checker("character()")

        with pytest.raises(
            SyntaxError, match="Found whitespace between keyword or value"
        ):
        ...
        with pytest.raises(
            SyntaxError, match="Found '=' at the beginning of arguments"
        ):
            UIForm.type_checker("character(=)")
        ...
        with pytest.raises(
            SyntaxError, match="No keyword passed for argument before '=' symbol"
        ):
            UIForm.type_checker("character(default='a',=)")
        ...
        with pytest.raises(
            SyntaxError, match="Found comma at the beginning of arguments"
        ):
            UIForm.type_checker("character(,)")
        ...
        with pytest.raises(
            SyntaxError, match="No value passed for keyword argument 'default'"
        ):
            UIForm.type_checker("character(default=,)")

        with pytest.raises(
            NameError, match="'a' is not defined. Perhaps quotes were missed"
        ):
            UIForm.type_checker("character(default=a)")
        ...
        with pytest.raises(
            ValueError,
            match="Question of type 'character' take at most 2 arguments"
            " but 3 arguments were passed",
        ):
            UIForm.type_checker("character('a','b','c')")

        with pytest.raises(
            SyntaxError, match="Found positional argument after keyword argument"
        ):
            UIForm.type_checker("character(default='a', 'b')")

        with pytest.raises(
            ValueError,
            match="Unknown parameter 'unknown' for question type 'character'",
        ):
            UIForm.type_checker("character(unknown = 'a')")

        with pytest.raises(
            SyntaxError, match="Got multiple values for parameter 'default'"
        ):
        ...
        with pytest.raises(
            ValueError, match="Invalid value ''ab'' for argument 'default'"
        ):
        ...
        with pytest.raises(
            ValueError, match="Invalid value ''a'' for argument 'default'"
        ):
            UIForm.type_checker("integer('a')")

        with pytest.raises(
            ValueError, match="Invalid value '2.0' for argument 'default'"
        ):
            UIForm.type_checker("integer(2.0)")

        with pytest.raises(
            ValueError, match="Invalid value ''2'' for argument 'default'"
        ):
        ...
        with pytest.raises(
            ValueError, match="Invalid value ''True'' for argument 'default'"
        ):
            UIForm.type_checker("boolean(default='True')")

        with pytest.raises(
            ValueError, match="Invalid value ''False'' for argument 'required'"
        ):
            UIForm.type_checker("boolean(required='False')")

    def test_validate_questionnaire(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        container = UIForm(
            pygame.Rect(100, 100, 200, 200),
    ...
    @pytest.mark.filterwarnings("ignore:Font data validation")
    @pytest.mark.filterwarnings("ignore:Image data validation")
    def test_validate_questionnaire_bad_values(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        container = UIForm(
            pygame.Rect(100, 100, 200, 200),
