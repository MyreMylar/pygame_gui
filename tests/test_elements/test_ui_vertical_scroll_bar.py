...
class TestUIVerticalScrollBar:
    def test_creation(
        ...
        scroll_bar = UIVerticalScrollBar(
            relative_rect=pygame.Rect(100, 100, 30, 150),
    ...
    def test_check_update_sliding_bar(
        ...
        scroll_bar = UIVerticalScrollBar(
            relative_rect=pygame.Rect(0, 0, 30, 150),
            visible_percentage=0.7,
            manager=default_ui_manager,
        )
    ...
    def test_process_event(
        ...
        assert (
            scroll_bar.process_event(pygame.event.Event(pygame.MOUSEWHEEL, {"y": 0.5}))
            is True
        )
    ...
    def test_rebuild_from_theme_data_non_default(
        ...
        manager = UIManager(
            (800, 600),
            os.path.join(
                "tests", "data", "themes", "ui_vertical_scroll_bar_non_default.json"
            ),
        )
    ...
    def test_rebuild_from_theme_data_no_arrow_buttons(
        ...
        manager = UIManager(
            (800, 600),
            os.path.join(
                "tests", "data", "themes", "ui_vertical_scroll_bar_no_arrows.json"
            ),
        )
    ...
    def test_rebuild_from_theme_data_bad_values(
        ...
        manager = UIManager(
            (800, 600),
            os.path.join(
                "tests", "data", "themes", "ui_vertical_scroll_bar_bad_values.json"
            ),
        )
    ...
    def test_set_position(
        ...
        scroll_bar = UIVerticalScrollBar(
            relative_rect=pygame.Rect(80, 100, 30, 200),
        ...
        default_ui_manager.process_events(
            pygame.event.Event(pygame.MOUSEBUTTONDOWN, {"button": 1, "pos": (215, 205)})
        )
        ...
        default_ui_manager.process_events(
            pygame.event.Event(pygame.MOUSEBUTTONDOWN, {"button": 1, "pos": (215, 395)})
        )
        # if we successfully clicked on the moved scroll bar then this button should be True
        assert scroll_bar.bottom_button.held is True

        default_ui_manager.process_events(
            pygame.event.Event(pygame.MOUSEBUTTONDOWN, {"button": 1, "pos": (215, 250)})
        )
        # if we successfully clicked on the moved scroll bar then this button should be True
    ...
    def test_set_relative_position(
        ...
        # try to click on the scroll bar's top button
        default_ui_manager.process_events(
            pygame.event.Event(pygame.MOUSEBUTTONDOWN, {"button": 1, "pos": (115, 105)})
        )
        # if we successfully clicked on the moved scroll bar then this button should be True
        assert scroll_bar.top_button.held is True

        default_ui_manager.process_events(
            pygame.event.Event(pygame.MOUSEBUTTONDOWN, {"button": 1, "pos": (115, 295)})
        )
        # if we successfully clicked on the moved scroll bar then this button should be True
        assert scroll_bar.bottom_button.held is True

        default_ui_manager.process_events(
            pygame.event.Event(pygame.MOUSEBUTTONDOWN, {"button": 1, "pos": (115, 150)})
        )
        # if we successfully clicked on the moved scroll bar then this button should be True
        assert scroll_bar.sliding_button.held is True
    ...
    def test_set_dimensions(
        ...
        scroll_bar = UIVerticalScrollBar(
            relative_rect=pygame.Rect(0, 100, 30, 200),
            visible_percentage=0.25,
            manager=default_ui_manager,
        )
        ...
        default_ui_manager.process_events(
            pygame.event.Event(pygame.MOUSEBUTTONDOWN, {"button": 1, "pos": (40, 195)})
        )
    ...
    def test_set_scroll_from_start_percentage(
        ...
        scroll_bar = UIVerticalScrollBar(
            relative_rect=pygame.Rect(0, 0, 30, 146),
