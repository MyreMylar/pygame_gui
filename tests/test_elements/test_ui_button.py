...
class TestUIButton:
    def test_creation(
        ...
        button_with_tt_kwargs = UIButton(
            relative_rect=pygame.Rect(100, 100, 150, 30),
            text="Test Button",
            manager=default_ui_manager,
    ...
    def test_set_any_images_from_theme(
    ...
    def test_on_hovered(
        ...
        confirm_on_hovered_event_fired = any(
            (
    ...
    def test_update(
        ...
        redraw_queue_length_pre_update = len(
            button.drawable_shape.states_to_redraw_queue
        )
        button.update(0.01)
        redraw_queue_length_post_update = len(
            button.drawable_shape.states_to_redraw_queue
        )
    ...
    def test_set_relative_position(
        ...
        assert button.rect.topleft == (
            250,
            130,
        ) and button.drawable_shape.containing_rect.topleft == (250, 130)
    ...
    def test_set_position(
        ...
        button_2 = UIButton(
            relative_rect=pygame.Rect(200, 0, 50, 30),
            text="Test Button",
            tool_tip_text="This is a test of the button's tool tip functionality.",
            container=test_container,
            manager=default_ui_manager,
        ...
        assert button.relative_rect.topleft == (
            50,
            -70,
        ) and button.drawable_shape.containing_rect.topleft == (150, 30)
    ...
    def test_set_dimensions(
        ...
        assert (
            button.drawable_shape.containing_rect.width == 250
            and button.drawable_shape.containing_rect.height == 60
        )
    ...
    def test_process_event_double_click(
        ...
        consumed_event_1 = button.process_event(
            pygame.event.Event(
        ...
        consumed_event_2 = button.process_event(
            pygame.event.Event(
                pygame.MOUSEBUTTONDOWN,
                {"button": pygame.BUTTON_LEFT, "pos": button.rect.center},
            )
        ...
        for event in pygame.event.get():
            if (
                event.type == pygame_gui.UI_BUTTON_DOUBLE_CLICKED
                and event.ui_element == button
            ):
    ...
    def test_process_event_mouse_button_down(
        self,
        _init_pygame: None,
        default_ui_manager: UIManager,
        _display_surface_return_none: None,
        ...
        # process a mouse button down event
        processed_down_event = button.process_event(
            pygame.event.Event(pygame.MOUSEBUTTONDOWN, {"button": 1, "pos": (50, 25)})
        )

        assert (
            processed_down_event is True
            and button.held is True
            and button.tool_tip is None
        )
    ...
    def test_process_event_mouse_button_up_inside(
        ...
        processed_up_event = button.process_event(
            pygame.event.Event(pygame.MOUSEBUTTONUP, {"button": 1, "pos": (50, 25)})
        )

        assert (
            processed_down_event is True
            and processed_up_event is True
            and button.held is False
            and button.pressed_event is True
            and default_ui_manager.focused_set is None
        )
    ...
    def test_on_self_event(
        ...
        command_dict = {
            pygame_gui.UI_BUTTON_START_PRESS: test_function,
    ...
    def test_on_self_event_no_params(
        self,
        _init_pygame: None,
        default_ui_manager: UIManager,
        _display_surface_return_none,
    ...
    def test_set_text(
        ...
        assert (
            empty_queue == 0
            and full_queue != 0
            and button.drawable_shape.theming["text"] == "Ipsum"
            and button.text == "Ipsum"
        )

        dynamic_width_button = UIButton(
    ...
    def test_set_text_same(
        ...
        assert (
            empty_queue == 0
            and still_empty_queue == 0
            and button.drawable_shape.theming["text"] == "Test Button"
            and button.text == "Test Button"
        )

    def test_set_hold_range(
        self,
        _init_pygame: None,
        default_ui_manager: UIManager,
        _display_surface_return_none,
    ...
    def test_in_hold_range_outside_button(
        self,
        _init_pygame: None,
        default_ui_manager: UIManager,
        _display_surface_return_none,
    ...
    def test_out_of_hold_range_when_zeroed(
        self,
        _init_pygame: None,
        default_ui_manager: UIManager,
        _display_surface_return_none,
    ...
    def test_switching_object_ids(self, _init_pygame, _display_surface_return_none):
        ...
        button.change_object_id(
            ObjectID(object_id=None, class_id="@rectangular_button")
        )
    ...
    def test_rebuild_from_changed_theme_data_bad_values_2(
        self, _init_pygame, _display_surface_return_none
    ):
            ...
            with pytest.warns(UserWarning, match="Image data validation errors"):
                with pytest.warns(UserWarning, match="Unable to find image with id"):
                    manager = UIManager(
                        (800, 600),
                        os.path.join(
                            "tests", "data", "themes", "ui_button_bad_values_2.json"
                        ),
                    )
    ...
    def test_rebuild_anchors_dynamic_dimensions(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        button = UIButton(
            relative_rect=pygame.Rect(10, 10, 150, -1),
            text="Test Button",
            tool_tip_text="This is a test of the button's tool tip functionality.",
            manager=default_ui_manager,
    ...
    def test_hide_and_show_of_disabled_button(
        self, _init_pygame, _display_surface_return_none
    ):
        manager = UIManager((800, 600))
        button = UIButton(
    ...
    def test_class_theming_id(self, _init_pygame, _display_surface_return_none):
        ...
        button = UIButton(
            ...
            object_id=pygame_gui.core.ObjectID(
                object_id="#test_object_1", class_id="@test_class"
            ),
        )

        assert button.combined_element_ids == [
            "#test_object_1",
            "@test_class",
            "button",
        ]
        ...
        test_container = UIContainer(
            relative_rect=pygame.Rect(100, 100, 300, 60), manager=manager
        )
    ...
    def test_change_locale(
        ...
        dynamic_width_button = UIButton(
            relative_rect=pygame.Rect(100, 100, -1, 30),
            text="Test Button",
            tool_tip_text="This is a test of the button's tool tip functionality.",
            manager=default_ui_manager,
        )
    ...
    def test_auto_scale_images_functionality(
        self, _init_pygame, _display_surface_return_none
    ):
        ...
        assert scaled_wide.get_size() == (
            50,
            50,
        )  # Should fit height, maintain square aspect

        scaled_tall = UIButton._scale_image_to_fit(test_image, (50, 100))
        assert scaled_tall.get_size() == (
            50,
            50,
        )  # Should fit width, maintain square aspect
        ...
        assert (
            scaled_zero == test_image
        )  # Should return original when target has zero dimension

    def test_auto_scale_images_theme_parameter(
        self, _init_pygame, _display_surface_return_none
    ):
        """Test that auto_scale_images theme parameter is loaded correctly."""
        # Create theme with auto_scale_images enabled
        theme_data = {"button": {"misc": {"auto_scale_images": "1"}}}
    ...
    def test_auto_scale_images_disabled_by_default(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
    ...
    def test_auto_scale_images_with_rectangular_image(
        self, _init_pygame, _display_surface_return_none
    ):
    ...
    def test_auto_scale_images_aspect_ratio_preservation(
        self, _init_pygame, _display_surface_return_none
    ):
        ...
        for image_size, expected_ratio in test_cases:
            ...
            for target_size in target_sizes:
                ...
                assert (
                    abs(actual_ratio - expected_ratio) < 0.02
    ...
    def test_auto_scale_images_integration_with_theming(
        self, _init_pygame, _display_surface_return_none
    ):
        ...
        theme_data = {
            "button": {
                "misc": {"auto_scale_images": "1"},
                "images": {"normal_image": {"path": "tests/data/images/splat.png"}},
            }
        }
        ...
        try:
            ...
            small_button = UIButton(
                relative_rect=pygame.Rect(100, 100, 50, 50),
                text="Small",
                manager=manager,
            )

            large_button = UIButton(
                relative_rect=pygame.Rect(200, 100, 200, 150),
                text="Large",
                manager=manager,
            )
    ...
    def test_auto_scale_images_rebuild_behavior(
        self, _init_pygame, _display_surface_return_none
    ):
    ...
    def test_auto_scale_images_edge_cases(
        self, _init_pygame, _display_surface_return_none
    ):
    ...
    def test_auto_scale_images_performance(
        self, _init_pygame, _display_surface_return_none
    ):
    ...
    def test_multi_image_current_state_access(
        self, _init_pygame, _display_surface_return_none
    ):
    ...
    def test_multi_image_with_auto_scaling(
        self, _init_pygame, _display_surface_return_none
    ):
    ...
    def test_multi_image_state_transitions(
        self, _init_pygame, _display_surface_return_none
    ):
    ...
    def test_multi_image_mode_switching(
        self, _init_pygame, _display_surface_return_none
    ):
    ...
    def test_multi_image_layer_ordering(
        self, _init_pygame, _display_surface_return_none
    ):
    ...
    def test_theme_switching_cache_clearing(
        self, _init_pygame, _display_surface_return_none
    ):
    ...
    def test_image_positioning_multi_image_mode(
        ...
        try:
            ...
            button.select()
            assert (
                button.get_current_image_positions() == expected_normal_positions
    ...
    def test_image_positioning_validation_valid_values(
        self, _init_pygame, _display_surface_return_none
    ):
    ...
    @pytest.mark.filterwarnings("ignore:Theme validation found")
    def test_image_positioning_validation_invalid_values(
        self, _init_pygame, _display_surface_return_none
    ):
        ...
        for i, theme_data in enumerate(invalid_themes):
            ...
            try:
                ...
                position_errors = [
                    error for error in errors if "position" in error.lower()
                ]
                assert (
                    len(position_errors) > 0
                ), f"Theme {i} should have position-related errors"

            finally:
                os.unlink(theme_file)

    def test_image_positioning_state_fallbacks(
        self, _init_pygame, _display_surface_return_none
    ):
    ...
    def test_image_positioning_api_methods(
        ...
        try:
            ...
            assert len(button.get_current_image_positions()) == len(
                button.get_current_images()
            )
    ...
    def test_image_positioning_theme_loading_and_storage(
        ...
        try:
            ...
            normal_details = manager.ui_theme.get_image_details(
                "normal_image", ["button"]
            )
            assert normal_details[0]["position"] == (0.0, 0.0)

            hovered_details = manager.ui_theme.get_image_details(
                "hovered_image", ["button"]
            )
            assert hovered_details[0]["position"] == (1.0, 1.0)

            selected_details = manager.ui_theme.get_image_details(
                "selected_image", ["button"]
            )
            assert selected_details[0]["position"] == (0.5, 0.5)

            disabled_details = manager.ui_theme.get_image_details(
    ...
    def test_image_positioning_state_transitions(
        self, _init_pygame, _display_surface_return_none
    ):
    ...
    def test_image_positioning_multi_image_format(
        self, _init_pygame, _display_surface_return_none
    ):
    ...
    def test_image_positioning_fallback_behavior(
        self, _init_pygame, _display_surface_return_none
    ):
    ...
    def test_image_positioning_default_center_position(
        self, _init_pygame, _display_surface_return_none
    ):
